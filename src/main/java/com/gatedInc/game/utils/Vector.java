...
public class Vector {
     ...
     *
     * @param degrees degrees of counterclockwise rotation
     ...
     *
     * @param vec
     * @return signed angle from this to vec, between -180 and 180 (positive for counterclockwise)
     */
    public double angleWith(Vector vec) {
        Vector vec1 = this.normalize();
        Vector vec2 = vec.normalize();
        double cos = vec1.dotProduct(vec2);
        double theta = Math.toDegrees(Math.acos(Math.min(1, Math.max(-1, cos))));
        return
     ...
     *
     * @return signed angle from the horizontal to this, between -180 and 180 (positive for counterclockwise)
     */
    public double angle() {
